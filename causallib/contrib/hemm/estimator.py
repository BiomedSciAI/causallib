...
from causallib.contrib.hemm.hemm import HEMM


class HemmEstimator(IndividualOutcomeEstimator):
    ...
    def __init__(self, 
        ...
        Args:
            ...
            outcomeModel (str): 'linear' to specify a linear outcome function.
                                Or pass another Torch.model as the outcome model.
                                Several outcome models are available in the `hemm_outcome_models.py` module.
        ...
        self._model = HEMM(D_in, K, homo, mu, std, bc, lamb, spread=spread, outcomeModel=outcome_model)
        self._model.double()
    ...
    def estimate_individual_outcome(self, X, a, treatment_values=None, soft=True):
        ...
        """
        ys = self._model.estimate_individual_outcomes(self._as_tensor(X), self._as_tensor(a),
    ...
    def get_groups(self, X, a):
        ...
        """
        groups = self._model.get_groups(self._as_tensor(X), self._as_tensor(a))
    ...
    def get_groups_proba(self, X, a, log=False):
        ...
        groups = self._model.get_groups_proba(X, a, log=log)
    ...
    def get_groups_effect(self, X=None, a=None):
        ...
        group_effect = self._model.get_groups_effect()
        return group_effect
    ...
    def group_sizes(self, X, a):
        ...
        """
        return self._model.group_sizes(self._as_tensor(X), self._as_tensor(a))

    def fit(self, X, a, y, sample_weight=None, X_dev=None, a_dev=None, y_dev=None):
        ...
        Args:
            ...
            X_dev (torch.Tensor | pd.DataFrame | np.ndarray): Covariate matrix of features for model verification.
            a_dev (torch.Tensor | pd.Series | np.ndarray): Treatment assignment for model verification.
            y_dev (torch.Tensor | pd.Series | np.ndarray): Observed outcomes for model verification.

        Returns:
            IndividualOutcomeEstimator: A causal weight model with an inner learner fitted.
        """
        ltype = 'log' if self._elbo_loss else None
        if X_dev is not None and a_dev is not None and y_dev is not None:
            X_dev = self._as_tensor(X_dev)
            a_dev = self._as_tensor(a_dev)
            y_dev = self._as_tensor(y_dev)
            dev = (X_dev, a_dev, y_dev)
        else:
            dev = None
