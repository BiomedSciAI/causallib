...
def prevalence_symmetric(tree, alpha=0.1):
    ...
    non_violating_nodes = leaf_summary['node_index'][
        leaf_summary['pscore'].between(lower_cutoff, upper_cutoff)].tolist()
...
class BICauseTree(IndividualOutcomeEstimator):
    ...
    def estimate_population_outcome(self, X, a, y=None, discard_violating_samples=True, treatment_values=None,
                                    agg_func="mean"):
    ...
    def estimate_individual_outcome(self, X, a, y=None, same_dim_as_input=True, treatment_values=None,
                                    predict_proba=None):
...
class PropensityImbalanceStratification(PopulationOutcomeEstimator):
    def __init__(
        ...
        if self.multiple_hypothesis_test_method is not None and self.multiple_hypothesis_test_alpha is None:
            raise ValueError(
                "multiple_hypothesis_test_alpha is None, must be set to a value if multiple_hypothesis_test_method is not None")
    ...
    def _recurse_over_build(self, X, a):
        ...
        self.subtree_ = (
            PropensityImbalanceStratification(**self_params, max_depth=next_max_depth, _parent_=self)._build_tree(
                X_left, a_left),
            PropensityImbalanceStratification(**self_params, max_depth=next_max_depth, _parent_=self)._build_tree(
                X_right, a_right),
        )
    ...
    def find_non_positivity_violating_leaves(self, positivity_filtering_method,
        ...
        """
        non_violating_nodes_idx = positivity_filtering_method(self,
                                                              **positivity_filtering_kwargs)  # returns the non-violating leaves
